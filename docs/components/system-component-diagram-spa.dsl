  ...
  model {
    godstracker = softwareSystem "GOD's TRACKER" "Main system" {
      ...
      spa = container "Single Page Application" "Angular 18 Frontend" {
        ...
        identity = component "Identity" "Manages user authentication, JWT storage, and session roles"
        billing = component "Billing" "Handles subscription plans, invoice history, and Stripe integration"
        fleet = component "Fleet" "Manages vehicle inventory, groups, and assignment of drivers"
        telemetry = component "Telemetry" "Visualizes real-time GPS coordinates and sensor data streams"
        alerting = component "Alerting" "Manages notification preferences and security event logs"
        commands = component "Commands" "Interface for remote vehicle actions (engine lock, reset)"
        query = component "Query" "Handles complex searches, filtering, and data retrieval for history"
      }
    }
